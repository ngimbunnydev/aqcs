...
$.fn.gridEditor = function( options ) {
    ...
    self.each(function(baseIndex, baseElem) {
        ...
        on: { 
            click: function() {
                //$(this).closest('.column').css('background-image', 'url(http://placekitten.com/g/300/300)');
                $(this).closest('.column').find('.ge-content').html('<p>New Content</p>');
            }
        }
    }, 
    ...
    /* Example:
        ...
        function setup() {
            ...
            $.each(settings.new_row_layouts, function(j, layout) {
                var btn = $('<a class="btn btn-sm btn-primary btn-white" />')
            ...
            var layoutDropdown = $('<div class="dropdown pull-right ge-layout-mode">' +
                '<button type="button" class="btn btn-sm btn-primary dropdown-toggle" data-toggle="dropdown" style="height:29px"><span>Desktop</span></button>' +
