@php 
  ...
  $hasPermisson = checkpermission("product-cost", $args['userinfo']);
    ...
    foreach($results as $row){
      ...
      ///
      $spreadsheet->getActiveSheet()
                    ->setCellValue($start_cell.$content_row, $branch[$row['branch_id']])
                    ->setCellValue($cell_b.$content_row, $using_airtype['title'])
                    ->setCellValue($cell_c.$content_row, $using_airtype['standard_qty'])
                    ->setCellValue($cell_d.$content_row, $row['qty']);
