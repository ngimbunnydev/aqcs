...
class TableController extends Controller
{
    private $args;
	private $model;
    private $fprimarykey='table_id';
    private $tbltranslate='';
    private $dflang;
    private $request;
    private $rcdperpage=-1; #record per page#
    private $obj_info=['name'=>'table','title'=>'Table/Room','routing'=>'admin.controller','icon'=>'<i class="fa fa-chair" aria-hidden="true"></i>'];
    ...
    $this->obj_info['title'] = __('label.lb261');
        $this->args = $args;
    ...
    public function sfp($request, $results)
    {
        ...
        $allowed = array('title', 'table_id'); // add allowable columns to sort on
        $sort = in_array($request->input('sort'), $allowed) ? $request->input('sort') : 'table_id'; // if user type in the url a column that doesnt exist app will default to id
   ...
   public function delete(Request $request, $id=0)
    ...
    public function ajaxreturn(Request $request){
        ...
        $paymentmethod=$this->model->orderBy($this->fprimarykey, 'desc')->pluck('title', 'table_id');
