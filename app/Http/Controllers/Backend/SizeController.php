...
class SizeController extends Controller
{
    private $args;
	private $model;
    private $fprimarykey='s_id';
    private $tbltranslate='';
    private $dflang;
    private $request;
    private $rcdperpage=-1; #record per page#
    private $obj_info=['name'=>'size','title'=>'Size','routing'=>'admin.controller','icon'=>'<i class="fa fa-ruler-vertical" aria-hidden="true"></i>'];
	...
	public function __construct(array $args){ //public function __construct(Array args){
      ...
      $this->obj_info['title'] = __('label.lb27');
        $this->protectme = [  
    ...
    public function listingModel()
    {
        ...
        return $this->model->select(\DB::raw(   $this->fprimarykey." AS id, 
                                                    JSON_UNQUOTE(title->'$.".$this->dflang[0]."') AS title, pic"
                                                )
                                        );
    } /*../function..*/
    ...
    public function sfp($request, $results)
    {
        ...
        $sort = in_array($request->input('sort'), $allowed) ? $request->input('sort') : 's_id'; // if user type in the url a column that doesnt exist app will default to id
   ...
   public function delete(Request $request, $id=0)
    ...
    public function ajaxreturn(){
        ...
        $size = $this->listingmodel()->where('trash', '!=', 'yes');
        $size=$size->pluck('title','id');
        $size = cmb_listing($size,[],'','');

        $return = [
                    'callback' => 'sizetocombo',
                    'container' => '#sizes',
                    'data' => $size,
                    'message' => ''
                ];
