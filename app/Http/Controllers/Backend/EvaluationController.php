...
class EvaluationController extends Controller
{
    private $args;
	private $model;
    private $fprimarykey='evaluation_id';
    private $tbltranslate='';
    private $dflang;
    private $request;
    private $rcdperpage=-1; #record per page#
    private $obj_info=['name'=>'evaluation','title'=>'Evaluation','routing'=>'admin.controller','icon'=>'<i class="fa fa-star-half-alt" aria-hidden="true"></i>'];
	...
	public function __construct(array $args){ //public function __construct(Array args){
        ...
        $this->obj_info['title'] = __('label.lb23');
        $this->args = $args;
    ...
    public function sfp($request, $results)
    {
        ...
        $allowed = array('title', 'evaluation_id'); // add allowable columns to sort on
        $sort = in_array($request->input('sort'), $allowed) ? $request->input('sort') : 'evaluation_id'; // if user type in the url a column that doesnt exist app will default to id
