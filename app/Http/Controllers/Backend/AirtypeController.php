...
class AirtypeController extends Controller
{
    private $args;
	private $model;
    private $fprimarykey='airtype_id';
    private $tbltranslate='';
    private $dflang;
    private $request;
    private $rcdperpage=-1; #record per page#
    private $obj_info=['name'=>'airtype','title'=>'Air Type','routing'=>'admin.controller','icon'=>'<i class="fa fa-fan" aria-hidden="true"></i>'];
    ...
    $this->obj_info['title'] = __('label.lb17');
        $this->args = $args;
		$this->model = new Airtype;
    ...
    public function sfp($request, $results)
    {
        ...
        $allowed = array('title', 'airtype_id'); // add allowable columns to sort on
        $sort = in_array($request->input('sort'), $allowed) ? $request->input('sort') : 'airtype_id'; // if user type in the url a column that doesnt exist app will default to id
               ...
               if($isupdate)
                {
                    ...
                    $rules['code.'.$i]      = 'required|distinct|unique:'.$this->model->gettable().',code,'.$request->input($this->fprimarykey)[0].','.$this->fprimarykey;
                    //JSON_UNQUOTE(title->'$.".$this->dflang[0]."') AS title
                ...
                else
                {
                    ...
                    $rules['code.'.$i]       = 'required|distinct|unique:'.$this->model->gettable().',code';
    ...
    public function setinfo($request, $isupdate=false){
        ...
        for($i=0; $i<$numrecord; $i++)
        {
            ...
            $record = [
                ...
                'airtype_index' => !empty($request->input('airtype_index')[$i])?$request->input('airtype_index')[$i]:0,
                'title' => json_encode($title),
                'code' => !empty($request->input('code')[$i])?$request->input('code')[$i]:'',
                'standard_qty'   => !empty($request->input('standard_qty')[$i])?$request->input('standard_qty')[$i]:0,
                'unit'   => !empty($request->input('unit')[$i])?$request->input('unit')[$i]:'',
                'color'   => !empty($request->input('color')[$i])?$request->input('color')[$i]:'',
                'noted'   => !empty($request->input('noted')[$i])?$request->input('noted')[$i]:'',
   ...
   public function delete(Request $request, $id=0)
    ...
    public function ajaxreturn(Request $request){
        ...
        $paymentmethod=$this->model->orderBy($this->fprimarykey, 'desc')->pluck('title', 'airtype_id');
