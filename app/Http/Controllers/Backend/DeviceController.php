...
class DeviceController extends Controller
{
    private $args;
	private $model;
    private $fprimarykey='device_id';
    private $tbltranslate='';
    private $dflang;
    private $request;
    private $rcdperpage=-1; #record per page#
    private $obj_info=['name'=>'device','title'=>'Device','routing'=>'admin.controller','icon'=>'<i class="fa fa-hdd" aria-hidden="true"></i>'];
    ...
    $this->obj_info['title'] = __('label.lb16');
        $this->args = $args;
    ...
    public function default()
    {
        ...
        return ['js_config'=>$js_config, 'location' => $location];
    } /*../function..*/
    ...
    public function sfp($request, $results)
    {
        ...
        $allowed = array('title', 'device_id'); // add allowable columns to sort on
        $sort = in_array($request->input('sort'), $allowed) ? $request->input('sort') : 'device_id'; // if user type in the url a column that doesnt exist app will default to id
	...
	public function index(Request $request, $condition=[], $setting=[])
    {
    	...
    	return view('backend.v'.$this->obj_info['name'].'.index', compact('location'))
                ->with(['act' => 'index'])
               ...
               if($isupdate)
                {
                    ...
                    $rules['device_index.'.$i]      = 'required|distinct|unique:'.$this->model->gettable().',device_index,'.$request->input($this->fprimarykey)[0].','.$this->fprimarykey;
                ...
                else
                {
                    ...
                    $rules['device_index.'.$i]       = 'required|distinct|unique:'.$this->model->gettable().',device_index';
    ...
    public function setinfo($request, $isupdate=false){
        ...
        for($i=0; $i<$numrecord; $i++)
        {
            ...
            $record = [
                ...
                'device_index' => !empty($request->input('device_index')[$i])?$request->input('device_index')[$i]:'',
                'location_id'   => !empty($request->input('location_id')[$i])?$request->input('location_id')[$i]:0,
                'title' => json_encode($title),
                'code' => !empty($request->input('code')[$i])?$request->input('code')[$i]:'',
                'model' => !empty($request->input('model')[$i])?$request->input('model')[$i]:'',
                'phonenum' => !empty($request->input('phonenum')[$i])?$request->input('phonenum')[$i]:'',
