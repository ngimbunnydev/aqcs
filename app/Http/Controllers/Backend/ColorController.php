...
class ColorController extends Controller
{
    private $args;
	private $model;
    private $fprimarykey='cl_id';
    private $tbltranslate='';
    private $dflang;
    private $request;
    private $rcdperpage=-1; #record per page#
    private $obj_info=['name'=>'color','title'=>'Color','routing'=>'admin.controller','icon'=>'<i class="fa fa-swatchbook" aria-hidden="true"></i>'];

    private $protectme;

    
	public function __construct(array $args){ //public function __construct(Array args){
    $this->obj_info['title'] = __('label.lb24');
        $this->protectme = [  
    ...
    public function listingModel()
    {
        ...
        return $this->model->select(\DB::raw(   $this->fprimarykey." AS id, 
                                                    JSON_UNQUOTE(title->'$.".$this->dflang[0]."') AS title, code, pic"
                                                )
                                        );
    } /*../function..*/
    ...
    public function sfp($request, $results)
    {
        ...
        $sort = in_array($request->input('sort'), $allowed) ? $request->input('sort') : 'cl_id'; // if user type in the url a column that doesnt exist app will default to id
    ...
    public function setinfo($request, $isupdate=false){
        ...
        for($i=0; $i<$numrecord; $i++)
        {
            ...
            {
                $title[$lang[0]]=!empty($request->input('title-'.$lang[0])[$i])?$request->input('title-'.$lang[0])[$i]:'General Warehouse';
   ...
   public function delete(Request $request, $id=0)
    ...
    public function ajaxreturn(){
        ...
        $color = $this->listingmodel()->where('trash', '!=', 'yes');
        $color=$color->pluck('title','id');
        $color = cmb_listing($color,[],'','');

        $return = [
                    'callback' => 'colortocombo',
                    'container' => '#colors',
                    'data' => $color,
                    'message' => ''
                ];
