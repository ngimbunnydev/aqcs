...
class PagesControllercopy extends Controller
{
    private $args;
	private $model;
    private $fprimarykey='p_id';
    private $dflang;
    private $request;
    private $rcdperpage=-1; #record per page, set negetive to get all record#
    private $obj_info=['name'=>'pages','title'=>'Pages','routing'=>'admin.controller','icon'=>'<i class="fa fa-columns" aria-hidden="true"></i>'];
    ...
    {
        #DEFIND MODEL#
        return $this->model->select(\DB::raw(   $this->fprimarykey." AS id, p_name,
    ...
    public function sfp($request, $results)
    {
        ...
        if ($request->has('title')) 
        {
            ...
            $results = $results->where('title', 'like', '%'.$qry.'%')->orwhere('title', 'like', '%'.$qry.'%');
    ...
    public function validation($request, $isupdate=false){
        // validate
            ...
            $rules = [
                        ...
                        'p_name' => 'required|regex:/([A-Za-z0-9 ])+/'
                    ];
    ...
    public function setinfo($request, $isupdate=false){
        ...
        /*For translate*/
        $translate=['metatitle', 'metakeyword', 'metades'];
        $title=[];
        $translateData=[];
        ...
        foreach (config('ccms.multilang') as $lang)
        {
            ...
            {
                $metatag[$field][$lang[0]]=$request->input($field.'-'.$lang[0]);
        ...
        $tableData = [
                ...
                'p_name' => str_clean($request->input('p_name')),
                'title' => json_encode($title),
                'contents' => $request->input('contents'),
                'metatag' => json_encode($metatag),
                'pm_id' => $request->input('pm_id'),
                'pm_pwd' => $request->input('pm_pwd'),
