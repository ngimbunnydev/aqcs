...
use App\Models\Backend\Reportdatetimebyweek;
...
class ReportdatetimeController extends Controller
{
    ...
    $this->obj_info['title'] = 'Date-Time';//__('label.livedata');
    $this->args = $args;
	$this->modelbyminute = new Reportdatetime;
    $this->modelbyhour = new Reportdatetimebyhour;
    $this->modelbyweek = new Reportdatetimebyweek;
	...
	public function index(Request $request, $condition=[], $setting=[])
    {
        ...
        if ($request->has('datatype') && !empty($request->input('datatype')))
        {
          ...
          elseif($request->input('datatype')=='week'){
            $this->tablename = 'airqty_byweek';
            $results = $this->listingmodel($this->modelbyweek, $this->tablename);
          }
          else{
            $results = $this->listingmodel($this->modelbyminute);
          }
